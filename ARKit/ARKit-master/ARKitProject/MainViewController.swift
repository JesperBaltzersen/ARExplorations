...
class MainViewController: UIViewController {
    ...
    func setupGestures(){
        ...
        let spinGesture = UIPanGestureRecognizer(target: self, action: #selector(spinNode(_:)))
        self.sceneView.addGestureRecognizer(spinGesture)
    }
    
    
    @objc func pinchToScale(_ gesture: UIPinchGestureRecognizer){
        print("pinchToScale hit")
        let location = gesture.location(in: sceneView)
        let hitTestResults = sceneView.hitTest(location)
        
        // test if hittest hits a node
        guard let nodeToScale = hitTestResults.first?.node else { return }
        print("hitTest result name: \(nodeToScale.name!)")
        
//        let nodesToScale =
        
//        print("number of child nodes: \(nodesToScale.count)")
//        nodesToScale.forEach{ node in
//            print("node name: \(node.name)")
//        }

        if gesture.state == .changed {
            scaleNode(node: nodeToScale, scale: gesture.scale)
            self.sceneView.scene.rootNode.enumerateChildNodes { (node, _) in
                if node.name == "text" {//&& nodeToScale.name == "hi"{
                    scaleNode(node: node, scale: gesture.scale)
                }
            }
//            nodeToScale.scale = SCNVector3Make(Float(pinchScaleX), Float(pinchScaleY), Float(pinchScaleZ))
    ...
    func scaleNode(node: SCNNode, scale: CGFloat) {
    ...
    // used to dial the speed of the pan up or down. The default value is in absolute pixels which is way too fast for most uses
    let panSpeedAdjustment : Float = 1000.0
    var curAngle : Float = 0.0
    
    @objc func spinNode(_ gesture: UIPanGestureRecognizer) {
        // determine how much to turn the object
        let pan = Float(gesture.translation(in: self.sceneView).x) / self.panSpeedAdjustment
        print("pan: \(pan)")
        
        // get the location of the first touch in the gesture
        let hitTestResult = sceneView.hitTest(gesture.location(in: sceneView))
        
        // Get the first available node below the touch point of the finger - we just assume this is the object we are looking for (this is the object you are looking for - says the jedi)
        guard let nodeToSpin = hitTestResult.first?.node else { return }
        // make sure the node can be cast to virualobject node - use VirtualObject extension 'isNodePartOfVirtualObject`
        // Get get nodeToSpin's top position in real world - i.e. boundingbox * scale.y
        // attach textnode to top of nodeToSpin by referencing nodeToSpin's boundingBox.max.y
        print(nodeToSpin.name)
        let isVirtualObjectNode = VirtualObject.isNodePartOfVirtualObject(nodeToSpin)
        if isVirtualObjectNode {
            let node = nodeToSpin as? VirtualObject
//        let text = node?.textNode
//        text.bou
        }
        ...
        if gesture.state == .changed{
            print(".changed: oldAngle: \(curAngle), newAngle: \(curAngle + pan)")
            curAngle += pan
            nodeToSpin.eulerAngles.y = curAngle
        ...
        if(gesture.state == .ended) {
            print("ended")
            curAngle = nodeToSpin.eulerAngles.y
        }
    }
